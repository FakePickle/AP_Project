...
public class BonusController implements Initializable {
    ...
    // Game Instance to access the game variables
    ...
    {
        try {
            BackgroundMedia = new MediaPlayer(new Media(Objects.requireNonNull(getClass().getResource("/com/example/stick_hero/Sound/BackGround_Bonus.mp3")).toURI().toString()));
        } catch (URISyntaxException e) {
            throw new RuntimeException(e);
        }
        BackgroundMedia.setCycleCount(MediaPlayer.INDEFINITE);
        ...
        GameLoop1 = new Timeline(new KeyFrame(Duration.millis(0.25), event -> {
            ...
            {
                if (MouseButton.PRIMARY == event1.getButton() && !characterMoving && !dead && !stickExtending && !stickFalling) {
                    Stick1.setOpacity(1);
            ...
            {
                if (MouseButton.PRIMARY == event1.getButton() && !characterMoving && !dead && !stickFalling) {
...
//                        System.out.println("You died");
                        ...
                        dead = false;
                        try {
                            Parent root = FXMLLoader.load(Objects.requireNonNull(getClass().getResource("endPagebonus.fxml")));
    ...
    private Timeline getStickExtendedTimeline()
    {
        AnimationTimer stickExtendedAnimation = new AnimationTimer()
        {
            ...
            public void handle(long now)
            {
                if (stickExtending && !characterMoving && !stickFalling)
                {
                    Stick1.setHeight(Stick1.getHeight() + (2 + (Instance * 0.5)));
                    Stick1.setY(Stick1.getY() - (2 + (Instance * 0.5)));
                }
                else if (stickFalling)
                {
                    ...
                    if (angle >= 90)
                    {
                        System.out.println("Stick has landed");
                        stop();
                        angle = 0;
    ...
    private Timeline getCharacterMovingTimeline()
    {
        AnimationTimer characterMovingAnimation = new AnimationTimer()
        {
            ...
            public void handle(long now)
            {
                ...
                {
                    Character.setLayoutX(Character.getLayoutX() + 2 + (Instance * 0.5));
                    if (counter % 5 == 0){
                        updateSprite();
                    }                    if(cherry_check()){
    ...
    {
        int pillar_diff = (int) (Pillar2.getX() - Pillar1.getX() + Pillar1.getWidth());
    ...
    private void nodeTransition()
    {
        AnimationTimer timer = new AnimationTimer() {
            ...
            public void handle(long now) {
                if (Pillar2.getX() > 0)
                {
                    Pillar2.setX(Pillar2.getX() - (2 + (Instance * 0.125)));
                    Pillar1.setX(Pillar1.getX() - (2 + (Instance * 0.125)));
                    Character.setLayoutX(Character.getLayoutX() - (2 + (Instance * 0.125)));
                    Stick1.setY(Stick1.getY() + (2 + (Instance * 0.125)));
                    cherry.setX(cherry.getX() - (2 + (Instance * 0.125)));
                    bonus_point.setX(bonus_point.getX() - (2 + (Instance * 0.125)));
                }
                else
                {
